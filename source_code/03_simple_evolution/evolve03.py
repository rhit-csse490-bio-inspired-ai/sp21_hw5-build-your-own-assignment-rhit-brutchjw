...
import possible_answers03 as pa

os.chdir('../..')
base_dir = os.getcwd()
viz_dir = os.path.join(base_dir, 'viz\\03_visualizations')
visualize.set_viz_dir(viz_dir)
train_dir = os.path.join(base_dir, 'data\\Train\\03_simple_color_shape')
training_images = []
average_accuracy = []
l = len(pa.answers)
...
def eval_genomes(genomes, config):
    ...
    for genome_id, genome in genomes:
        genome.fitness = 4*len(training_images)
        ...
        for train in training_images:
            ...
            for i in range(l):
                if train.answer == pa.answers.__getitem__(i):
                    genome.fitness -= (output[i] - 1) ** 2
                else:
                    genome.fitness -= (output[i] - 0) ** 2
